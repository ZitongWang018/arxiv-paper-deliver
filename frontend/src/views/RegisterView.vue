  ...
  <div class="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-primary-50 via-white to-purple-50 dark:from-gray-950 dark:via-gray-900 dark:to-primary-950">
    <div class="w-full max-w-md">
      <div class="text-center mb-8">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">开始使用 ArxivDigest 智能论文推荐</p>
      </div>
      ...
      <div class="card p-8">
        <form @submit.prevent="handleRegister" class="space-y-5">
          ...
          <div>
            ...
            <input v-model="password" type="password" required placeholder="至少6个字符" class="input-field" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">确认密码</label>
            <input v-model="confirmPassword" type="password" required placeholder="再次输入密码" class="input-field" />
          </div>
        ...
        <p class="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
          已有账号？
          <router-link to="/login" class="font-semibold text-primary-600 hover:text-primary-500 transition-colors">登录</router-link>
        </p>
      </div>
    </div>
  </div>
